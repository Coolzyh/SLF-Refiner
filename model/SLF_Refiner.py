...
#
#         # Encoding coarse slf image x
#         self.conv1 = conv_layer(1, 32, 5, stride=2, padding=2)         # shape: [batch_size, 32, K0/2, K1/2]
#
#         self.conv2 = nn.Conv2d(32, 64, 3, padding='same')             # shape: [batch_size, 64, K0/2, K1/2]
#         self.bn1 = nn.BatchNorm2d(64)
#         self.augmented_conv1 = AugmentedConv(in_channels=64, out_channels=64, kernel_size=3, dk=128, dv=16,
#                                              Nh=2)  # shape: [batch_size, 64, K0/2, K1/2]
#         # self.augmented_conv1 = nn.Conv2d(64, 64, 3, padding=1)  # shape: [batch_size, 64, K0/2, K1/2]
#         self.gamma1 = nn.Parameter(torch.zeros(1))
#         self.bn2 = nn.BatchNorm2d(64)
#
#         self.conv3 = conv_layer(64, 128, 3, padding='same')            # shape: [batch_size, 128, K0/2, K1/2]
#         self.maxpool1 = nn.MaxPool2d(2, 2)  # shape: [batch_size, 128, K0/4, K1/4]
#
#         self.conv4 = nn.Conv2d(128, 256, 3, padding=0)                 # shape: [batch_size, 256, K0/4-2, K1/4-2]
#         self.bn3 = nn.BatchNorm2d(256)
#         self.augmented_conv2 = AugmentedConv(in_channels=256, out_channels=256, kernel_size=3, dk=512, dv=64,
#                                                Nh=2)  # shape: [batch_size, 256, K0/4-2, K1/4-2]
#         # self.augmented_conv2 = nn.Conv2d(256, 256, 3, padding=1)  # shape: [batch_size, 256, K0/4-2, K1/4-2]
#         self.gamma2 = nn.Parameter(torch.zeros(1))
#         self.bn4 = nn.BatchNorm2d(256)
#
#         self.conv5 = conv_layer(256, 256, 3, padding='same')  # shape: [batch_size, 256, K0/4-2, K1/4-2]
#         self.maxpool2 = nn.MaxPool2d(2, 2)                             # shape: [batch_size, 256, K0/8-1, K1/8-1]
#         self.conv6 = conv_layer(256, 256, 3, padding='same')           # shape: [batch_size, 256, K0/8-1, K1/8-1]
#
#     def forward(self, x):
#         # x is the coarse slf image
#         out = self.conv1(x)
...
#         self.P = P  # Each node has P measurement positions
#         # input size (CxWxH) = (NxPxP)
#         self.conv1 = nn.Conv2d(self.N, 64, 3, padding='same')  # shape: [batch_size, 128, P, P]
#         self.bn1 = nn.BatchNorm2d(64)
#
#         self.augmented_conv1 = AugmentedConv(in_channels=64, out_channels=64, kernel_size=3, dk=128, dv=16,
#                                              Nh=2)  # shape: [batch_size, 128, P, P]
#         # self.augmented_conv1 = nn.Conv2d(64, 64, 3, padding=1)  # shape: [batch_size, 128, P, P]
#         self.bn2 = nn.BatchNorm2d(64)
#         self.gamma1 = nn.Parameter(torch.zeros(1))
#
#         self.conv2 = conv_layer(64, 128, 3, padding=0)  # shape: [batch_size, 256, P-2, P-2]
#         self.conv3 = conv_layer(128, 256, 3, padding=1)  # shape: [batch_size, 512, P-2, P-2]
#
#     def forward(self, x):
#         # during testing stage
#         # x is the RSS measurement
#         out = self.conv1(x)  # shape: [batch_size, 64, P, P]
#         attn = self.augmented_conv1(F.leaky_relu(self.bn1(out), 0.2))  # shape: [batch_size, 64, P, P]
#         out = torch.sigmoid(self.gamma1) * attn + (
#                 1 - torch.sigmoid(self.gamma1)) * out  # shape: [batch_size, 64, P, P]
#         out = F.leaky_relu(self.bn2(out), 0.2)
#         out = self.conv2(out)  # shape: [batch_size, 128, P-2, P-2]
...
#         self.K = K  # dimension of SLF image
#         # input: x, generator_x, encoder_x
#         # size: [1, K0, K1], 1024, [256, K0/8-1, K1/8-1]
#         self.conv1 = conv_layer(256, 256, 3, padding=1)
#         self.conv2 = conv_layer(512, 512, 3, padding=1)  # shape: [batch_size, 512, 4, 4]
#         self.conv3 = conv_layer(512, 1024, 3, padding=1)  # shape: [batch_size, 1024, 4, 4]
#         self.conv4 = conv_layer(1024, 2048, 1, padding=0)         # shape: [batch_size, 2048, 4, 4]
#         self.avgpool = nn.AdaptiveAvgPool2d(1)
#         self.dropout_linear = nn.Dropout(p=0.5)
#         self.linear_out = nn.Linear(2048, self.K[0] * self.K[1])
#
...
#         # generator_x: output of the generator
#         # encoder_x: output of the encode_x()
#         out = self.conv1(generator_x)                  # shape: [batch_size, 256, 4, 4]
#         out = torch.cat((out, encoder_x), dim=1)     # shape: [batch_size, 256+256, 4, 4]
#         out = self.conv2(out)      # shape: [batch_size, 512, 4, 4]
#         out = self.conv3(out)  # shape: [batch_size, 1024, 4, 4]
#         out = self.conv4(out)      # shape: [batch_size, 2048, 4, 4]
#         out = self.avgpool(out)    # shape: [batch_size, 2048, 1, 1]
#         out = torch.flatten(out, start_dim=1)
#         out = self.dropout_linear(out)
